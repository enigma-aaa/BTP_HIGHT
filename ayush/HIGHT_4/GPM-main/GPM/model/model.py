...
class Model(nn.Module):
    ...
    def encode_node(self, graph, nodes, params, mode):
        ...
        feat = graph.x
        # Check for node PE in multiple possible attribute names
        if hasattr(graph, 'pe') and graph.pe is not None:
            node_pe = graph.pe
        elif hasattr(graph, 'node_pe') and graph.node_pe is not None:
            node_pe = graph.node_pe
        elif hasattr(graph, 'lap_pe') and graph.lap_pe is not None:
            node_pe = graph.lap_pe
        else:
            node_pe = None
        ...
        if graph.edge_attr is not None:
            ...
            valid_mask = eids >= 0
            if valid_mask.any():
                # Create a mask for valid edges and pad with zeros for invalid ones
                eids_clean = torch.where(valid_mask, eids, torch.zeros_like(eids))
                e_feat = e_feat[eids_clean].to(device)
                # Zero out features for invalid edges - ensure mask is on same device
                valid_mask = valid_mask.to(device)
                e_feat = e_feat * valid_mask.unsqueeze(-1).float()
            else:
                # All edges are invalid, create zero features
                e_feat = torch.zeros(eids.shape + (e_feat.shape[-1],), device=device, dtype=e_feat.dtype)
        else:
            e_feat = None

        pattern_feat = self.pattern_encoder.encode_node(patterns, feat, node_pe, e_feat, eids, params)
