...
def main():
    # Define paths
    data_dir = os.path.join(os.path.dirname(__file__), '..', 'data', 'MoleculeNet_data', 'hiv')
    raw_file = os.path.join(data_dir, 'raw', 'HIV.csv')
    ...
    # Process the data
    processed_data = []
    for _, row in tqdm(df.iterrows(), total=df.shape[0], desc="Processing HIV data"):
        smiles = row['smiles']
