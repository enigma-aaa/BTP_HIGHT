    ...
    **kwargs
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            # Load non-LoRA trainables AFTER LoRA loading to ensure they don't get overridden
            print('Loading additional LLaVA weights...')
            non_lora_trainables = {(k[11:] if k.startswith('base_model.') else k): v for k, v in non_lora_trainables.items()}
            if any(k.startswith('model.model.') for k in non_lora_trainables):
                non_lora_trainables = {(k[6:] if k.startswith('model.') else k): v for k, v in non_lora_trainables.items()}
            # Additional mapping for base_model.model.model. prefix
            if any(k.startswith('base_model.model.model.') for k in non_lora_trainables):
                non_lora_trainables = {(k[21:] if k.startswith('base_model.model.model.') else k): v for k, v in non_lora_trainables.items()}
            model.load_state_dict(non_lora_trainables, strict=False)
            print('Model is loaded...')
        elif model_base is not None:
            # this may be mm projector only
            print('Loading LLaVA from base model...')
            if 'mpt' in model_name.lower() and "prompt" not in model_name.lower():
