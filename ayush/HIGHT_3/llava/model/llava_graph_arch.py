...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_mol(self, mol, compute_type=torch.bfloat16):
        ...
        try:
            device = next(self.get_model().get_graph_tower().parameters()).device
        except StopIteration:
            # If no parameters, use the device from the main model
            device = next(self.get_model().parameters()).device
        mol = mol.to(device)
        ...
        if "hlinear" in self.config.mm_projector_type.lower():
            node_features = h_node[:mol.num_part[0]]
            motif_features = h_node[mol.num_part[0]:mol.num_part[0]+mol.num_part[1]]
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            if (cur_input_ids == IMAGE_TOKEN_INDEX).sum() == 0:
                ...
                # Apply mm_projector to dummy feature
                # Use only the first projector layer (they're applied to different feature types in parallel, not sequentially)
                dummy_feature = graph_tower.dummy_feature.to(cur_input_embeds.device).to(cur_input_embeds.dtype)
                mm_projector_0 = self.get_model().mm_projector[0].to(cur_input_embeds.dtype)
                dummy_output = mm_projector_0(dummy_feature).to(cur_input_embeds.device)
                cur_input_embeds = cur_input_embeds + (0. * dummy_output).sum()
