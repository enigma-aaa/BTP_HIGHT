...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_mol(self, mol, compute_type=torch.bfloat16):
        ...
        try:
            if self.config.use_lap_pe:
                h_node = torch.cat([h_node,mol.lap_pe],dim=-1)
                # print(h_node.size())
        ...
        if "hlinear" in self.config.mm_projector_type.lower():
            ...
            global_features = torch.cat([_,mol.lap_pe[-1,:].view(_.size(0),-1)],dim=-1)
            # print(node_features.size(),motif_features.size(),global_features.size())
            graph_features = []
            graph_features.append(self.get_model().mm_projector[0].float()(node_features))
            if motif_features.size(0):
                graph_features.append(self.get_model().mm_projector[1].float()(motif_features))
            graph_features.append(self.get_model().mm_projector[2].float()(global_features))
