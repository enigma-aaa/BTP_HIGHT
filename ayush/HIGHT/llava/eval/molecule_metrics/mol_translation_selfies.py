...
def evaluate(input_file, verbose=False):
    ...
    print(f"{exact_match_score}, {bleu_score_self}, {levenshtein_score_smi}, {validity_score}")
...
python -m llava.eval.molecule_metrics.mol_translation_selfies \
    --input_file=eval_result/${GRAPH_TOWER}-retrosynthesis-5ep.jsonl  | tee 0318${GRAPH_TOWER}_retrosynthesis.log

# reagent prediction
python -m llava.eval.molecule_metrics.mol_translation_selfies \
    --input_file=eval_result/${GRAPH_TOWER}-reagent_pred-5ep.jsonl  | tee 0318${GRAPH_TOWER}_reagent_pred.log
    
# forward_pred
python -m llava.eval.molecule_metrics.mol_translation_selfies \
    --input_file=eval_result/${GRAPH_TOWER}-forward_pred-5ep.jsonl  | tee 0318${GRAPH_TOWER}_forward_pred.log
"""
