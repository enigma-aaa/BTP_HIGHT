...
def smiles2graph(smiles_string,motif=False,add_lap_pe=False)->Dict:
    """
    ...
    if add_lap_pe or True:
        ...
        edge_index, edge_weight = get_laplacian(
            torch.LongTensor(graph['edge_index']),
            normalization='sym',
            num_nodes=num_nodes,
        )
        ...
        pe = torch.from_numpy(eig_vecs[:, 1:8 + 1]).float()
        sign = -1 + 2 * torch.randint(0, 2, (8, ))
        # print("test",pe.size(),sign.size())
        pe *= sign
        graph['lap_pe']=pe
    return graph 
